...
const Refusal = () => {
  ...
  const getData = async () => {
    const res = await apiGet(`refusal/634055ceb696ad783a6f840e`).catch((err) =>
      console.log(err)
    );
  ...
  const updatePage = async () => {
    apiPut('refusal/634055ceb696ad783a6f840e', {
        ...
        <div className="row">
          <form className="update__forms" onSubmit={updatePage}>
            <h3 className="myaccount-content" style={{ textAlign: 'center' }}>
              {' '}
              REFUSAL APPEALS AND INADMISSIBILITY Page
            </h3>
            <p style={{ textAlign: 'center' }}>Add description here...</p>
